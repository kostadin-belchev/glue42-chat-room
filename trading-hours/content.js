...
(async () => {
  ...
  const setStatusToConnected = () => {
    ...
    if (typeof injectedGlue !== 'undefined') {
      tooltipElement.textContent = `Extension: Market 24h Clock Extension; Version: ${injectedGlue.version}; Username: ${injectedGlue.connection.resolvedIdentity.user}; GW URL: ${injectedGlue.config.connection.ws};`;
    }
  };
  ...
  window.injectedGlue = await Glue({ appManager: { mode: 'full' } });
